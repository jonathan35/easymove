...
$valid_from = $tour['validity_from'];
$valid_to = $tour['validity_to'];

$items = sql_read('select * from items where status=? and created >=? and created <=?', 'sss', array('Paid', $valid_from, $valid_to));
...
$tomorrow_time = strtotime(date('Y-m-d') . '+1 day');

if($tomorrow_time > $start_time){
    $start_time = $tomorrow_time;
    $start = date('Y-m-d', $tomorrow_time);
}

if($end_time < 1700000000){
...
/*----------- Left & Right Btn ----------*/
if((($year)*12+$month) == ((substr($start,0,4))*12+substr($start,5,2))){
    $left_btn = false;
}
$right_btn = true;
if((($year)*12+$month) == ((substr($end,0,4))*12+substr($end,5,2))){
    $right_btn = false;
}
...
<table id="wphp-cal">
    <tr class="panel-outter">
        ...
        <td class="panel-body" colspan="5" id="year-month" data-year="<?php echo $year?>" data-month="<?php echo $month?>" start-year="<?php echo substr($start,0,4)?>" start-month="<?php echo substr($start,5,2)?>" start-day="<?php echo substr($start,8,2)?>" end-year="<?php echo substr($end,0,4)?>" end-month="<?php echo substr($end,5,2)?>" end-day="<?php echo substr($end,8,2)?>">
        <!--id="cal-year-month" -->
