...
if($_POST['get_config_only']){
	...
	echo $id = $defender->encrypt('decrypt', $_SESSION['refresher']);
	echo $folder = $_SESSION['refresher_folder'];
	echo $php = $_SESSION['refresher_php'];
	echo $refresher = 'refresher'.$_SESSION['refresher'];
...
foreach((array)$cols as $colName => $colWidth){
	...
	<td class="<?php echo $refresher?>">
		...
		<div class="col-12" style="padding:0;">
			...
			<?php }elseif($type[$fieldName] == 'radio' || $type[$fieldName] == 'select' || $type[$fieldName] == 'datalist' || $type[$fieldName] == 'autosuggest'){ ?>
				<div class="blockInList">
					<?php if(!isset($list_sort)){?>
					<span class="span_label"><?php echo $str_convert->field_to_label($fieldName)?></span>
					<?php }?>
